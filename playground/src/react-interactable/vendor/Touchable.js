...
const TouchableMixin = {
  ...
  _handleQueryLayout: function( x, y, width, height, globalX, globalY ) {
    // don't do anything if UIManager failed to measure node
    if (!x && !y && !width && !height && !globalX && !globalY) {
      return;
		}
		// Sorry not pooled positions
		// this.state.touchable.positionOnActivate && Position.release(this.state.touchable.positionOnActivate);
		// Sorry not pooled dimensions
		// this.state.touchable.dimensionsOnActivate && BoundingDimensions.release(this.state.touchable.dimensionsOnActivate);
		
    this.state.touchable.positionOnActivate = {x: globalX, y: globalY};
    // $FlowFixMe
    this.state.touchable.dimensionsOnActivate = {width, height};
  },
...
const Touchable = {
  ...
  renderDebugView: ({ color, hitSlop }) => {
		// Removed, not available in react version on interactable
  }
};
